export async function fetchFeed (req) {
	...
	const channels = [
		...
		{ id: 'UC3d64X0YAFQHxAOXN49B10w', nickname: "matttt - comic & manga history", tags: [ 'comix' ] },
		{ id: 'UCHSbIfwVuec_BwUrYAqN_qw', nickname: "Sarah n Tuned", tags: [ 'cars', 'restoration' ] },
		{ id: 'UCshObcm-nLhbu8MY50EZ5Ng', nickname: "Benn Jordan", tags: [ 'science', 'education' ] },
		{ id: 'UC0pxFNDvJ8RNAjNosMPCx3Q', nickname: "Look Mum No Engine", tags: [ 'auto', 'restoration' ] },
		{ id: 'UCZYTClx2T1of7BRZ86-8fow', nickname: "SciShow", tags: [ 'education' ] },
	...
	await asyncBatch(channels, async ({ id, nickname, tags }) => {
		...
		try {
			...
			items.filter(f => !f.link.includes('short'))?.forEach(({ link, title, pubDate }) => {
